...
public class NecronomiconCleansingRitual extends NecronomiconRitual {
	...
	public NecronomiconCleansingRitual() {
		super("cleansing", 4, 0, 100000F, true, new Object[]{
				new ItemStack[]{
	...
	public boolean canCompleteRitual(World world, BlockPos pos, EntityPlayer player) {
		for (int x = pos.getX() - 24; x < pos.getX() + 25; x++)
			for (int z = pos.getZ() - 24; z < pos.getZ() + 25; z++) {
				...
				if (world.getBiome(pos1) instanceof IDarklandsBiome) return true;
			}
		return false;
	}
				...
				Scheduler.schedule(new ScheduledProcess(num * 2) {
					...
					public void execute() {
						...
						for (int y = 0; y < 256; y++) {
							...
							if (state ==
									ACBlocks.getInstance().darkstone.getBlock().getDefaultState())
								...
								world.setBlockState(pos1.up(y), Blocks.LEAVES.getDefaultState()
									...
									ACBlocks.getInstance().darklands_oak_wood.getBlock() ||
									state.getBlock() ==
											ACBlocks.getInstance().darklands_oak_wood_2.getBlock())
								world.setBlockState(pos1.up(y), Blocks.LOG.getDefaultState()
										.withProperty(BlockLog.LOG_AXIS,
												state.getValue(BlockLog.LOG_AXIS)), 2);
							else if (state.getBlock() ==
									ACBlocks.getInstance().abyssalnite_ore.getBlock())
								...
								world.setBlockState(pos1.up(y), Blocks.STONEBRICK.getDefaultState()
										.withProperty(BlockStoneBrick.VARIANT,
												BlockStoneBrick.EnumType.CHISELED), 2);
							else if (state.getBlock() ==
									ACBlocks.getInstance().cracked_darkstone_brick.getBlock())
								world.setBlockState(pos1.up(y), Blocks.STONEBRICK.getDefaultState()
										.withProperty(BlockStoneBrick.VARIANT,
												BlockStoneBrick.EnumType.CRACKED), 2);
							else if (state.getBlock() ==
								...
								world.setBlockState(pos1.up(y),
									...
									ACBlocks.getInstance().ritual_altar_darkstone.getBlock()
											.getDefaultState()) world.setBlockState(pos1.up(y),
									ACBlocks.getInstance().ritual_altar_stone.getBlock()
											.getDefaultState(), 2);
							else if (state ==
								...
								world.setBlockState(pos1.up(y), Blocks.STONE_SLAB.getDefaultState()
										.withProperty(BlockSlab.HALF,
												state.getValue(BlockSlab.HALF))
										.withProperty(BlockStoneSlab.VARIANT,
												BlockStoneSlab.EnumType.SMOOTHBRICK), 2);
							else if (state.getBlock() ==
								...
								world.setBlockState(pos1.up(y), Blocks.STONE_SLAB.getDefaultState()
										.withProperty(BlockSlab.HALF,
												state.getValue(BlockSlab.HALF))
										.withProperty(BlockStoneSlab.VARIANT,
												BlockStoneSlab.EnumType.COBBLESTONE), 2);
							else if (state.getBlock() ==
								...
								world.setBlockState(pos1.up(y),
										Blocks.DOUBLE_STONE_SLAB.getDefaultState(), 2);
							else if (state.getBlock() ==
									ACBlocks.getInstance().glowing_darkstone_bricks.getBlock())
								world.setBlockState(pos1.up(y), Blocks.STONEBRICK.getDefaultState()
										.withProperty(BlockStoneBrick.VARIANT,
												BlockStoneBrick.EnumType.CHISELED), 2);
							else if (state.getBlock() ==
								...
								world.setBlockState(pos1.up(y), Blocks.OAK_STAIRS.getDefaultState()
										.withProperty(BlockStairs.FACING,
												state.getValue(BlockStairs.FACING))
										.withProperty(BlockStairs.HALF,
												state.getValue(BlockStairs.HALF)), 2);
	...
	private Biome getRealBiome(Biome b) {
		if (b == ACBiomes.darklands || b == ACBiomes.darklands_plains) return Biomes.PLAINS;
		else if (b == ACBiomes.darklands_forest) return Biomes.FOREST;
		else if (b == ACBiomes.darklands_hills) return Biomes.EXTREME_HILLS;
		else if (b == ACBiomes.darklands_mountains) return Biomes.ICE_MOUNTAINS;
