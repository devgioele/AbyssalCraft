...
public abstract class NecronomiconRitual {
	 ...
	 */
	public NecronomiconRitual(String unlocalizedName, int bookType, int dimension,
							  float requiredEnergy, boolean requiresSacrifice,
							  Object... offerings) {
		...
		if (offerings.length < 8) {
			...
			System.arraycopy(offerings, 0, this.offerings, 0, offerings.length);
		} else {
	 ...
	 * @param offerings       Components used to perform the ritual, are consumed afterwards
	 */
	public NecronomiconRitual(String unlocalizedName, int bookType, float requiredEnergy,
							  Object... offerings) {
	 ...
	 * identical NBT tag compounds while checking if the sacrifices match (so a stricted Item
	 * match).
	 */
	...
	public void completeRitual(World world, BlockPos pos, EntityPlayer player) {
		if (!world.isRemote) {
			...
			AbyssalCraftAPI.getInternalMethodHandler()
					.completeRitualClient(pos, player, unlocalizedName);
		}
