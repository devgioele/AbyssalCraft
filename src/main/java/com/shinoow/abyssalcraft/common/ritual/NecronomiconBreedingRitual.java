...
public class NecronomiconBreedingRitual extends NecronomiconRitual {
	...
	public NecronomiconBreedingRitual() {
		super("breeding", 0, 500F, Items.WHEAT, Items.POTATO, Items.CARROT, Items.MELON,
	...
	public boolean canCompleteRitual(World world, BlockPos pos, EntityPlayer player) {
		...
		List<EntityAnimal> animals = world.getEntitiesWithinAABB(EntityAnimal.class,
				new AxisAlignedBB(pos).grow(16, 3, 16));
	...
	protected void completeRitualServer(World world, BlockPos pos, EntityPlayer player) {
		...
		for (Entity entity : entities) {
			if (entity instanceof EntityCow) cows.add((EntityCow) entity);
			if (entity instanceof EntityChicken) chickens.add((EntityChicken) entity);
			if (entity instanceof EntityPig) pigs.add((EntityPig) entity);
			if (entity instanceof EntitySheep) sheeps.add((EntitySheep) entity);
			if (entity instanceof EntityHorse) horses.add((EntityHorse) entity);
			if (entity instanceof EntityOcelot) ocelots.add((EntityOcelot) entity);
			if (entity instanceof EntityWolf) wolves.add((EntityWolf) entity);
			if (entity instanceof EntityRabbit) rabbits.add((EntityRabbit) entity);
		}
		if (!cows.isEmpty() && cows.size() >= 2)
			if ((cows.size() & 1) == 0) for (int i = 0; i < cows.size() / 2; i++) {
		...
		if (!chickens.isEmpty() && chickens.size() >= 2)
			if ((chickens.size() & 1) == 0) for (int i = 0; i < chickens.size() / 2; i++) {
		...
		if (!pigs.isEmpty() && pigs.size() >= 2)
			if ((pigs.size() & 1) == 0) for (int i = 0; i < pigs.size() / 2; i++) {
		...
		if (!sheeps.isEmpty() && sheeps.size() >= 2)
			if ((sheeps.size() & 1) == 0) for (int i = 0; i < sheeps.size() / 2; i++) {
		...
		if (!horses.isEmpty() && horses.size() >= 2)
			if ((horses.size() & 1) == 0) for (int i = 0; i < horses.size() / 2; i++) {
		...
		if (!ocelots.isEmpty() && ocelots.size() >= 2)
			if ((ocelots.size() & 1) == 0) for (int i = 0; i < ocelots.size() / 2; i++) {
		...
		if (!wolves.isEmpty() && wolves.size() >= 2)
			if ((wolves.size() & 1) == 0) for (int i = 0; i < wolves.size() / 2; i++) {
		...
		if (!rabbits.isEmpty() && rabbits.size() >= 2)
			if ((rabbits.size() & 1) == 0) for (int i = 0; i < rabbits.size() / 2; i++) {
		...
		if (horses.size() > 2 && getDeathChance(world.rand, horses.size()))
			horses.get(world.rand.nextInt(horses.size()))
					.attackEntityFrom(DamageSource.MAGIC, 200000);
		if (ocelots.size() > 2 && getDeathChance(world.rand, ocelots.size()))
			ocelots.get(world.rand.nextInt(ocelots.size()))
					.attackEntityFrom(DamageSource.MAGIC, 200000);
		if (wolves.size() > 2 && getDeathChance(world.rand, wolves.size()))
			wolves.get(world.rand.nextInt(wolves.size()))
					.attackEntityFrom(DamageSource.MAGIC, 200000);
		if (rabbits.size() > 2 && getDeathChance(world.rand, rabbits.size()))
			rabbits.get(world.rand.nextInt(rabbits.size()))
					.attackEntityFrom(DamageSource.MAGIC, 200000);
	}

	private boolean getDeathChance(Random rand, int num) {
		if (num < 5) return rand.nextInt(10) == 0;
		if (num > 5 && num < 10) return rand.nextInt(9) == 0;
		if (num > 10 && num < 15) return rand.nextInt(8) == 0;
		if (num > 15 && num < 20) return rand.nextInt(7) == 0;
		if (num > 20 && num < 25) return rand.nextInt(6) == 0;
		if (num > 25 && num < 30) return rand.nextInt(5) == 0;
		if (num > 30 && num < 35) return rand.nextInt(4) == 0;
		if (num > 35 && num < 40) return rand.nextInt(3) == 0;
		if (num > 40 && num < 45) return rand.nextInt(2) == 0;
		return true;
	}
