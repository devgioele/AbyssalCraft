...
public class NecronomiconCuringRitual extends NecronomiconRitual {
	...
	public NecronomiconCuringRitual() {
		super("curing", 4, 20000F, new ItemStack(ACItems.getInstance().antidote, 1, 1), null,
				new ItemStack(ACItems.getInstance().antidote, 1, 1), null,
				new ItemStack(ACItems.getInstance().antidote, 1, 1), null,
				new ItemStack(ACItems.getInstance().antidote, 1, 1));
		...
		if (world.provider.getDimension() != ACLib.dreadlands_id)
			for (int x = pos.getX() - 24; x < pos.getX() + 25; x++)
				for (int z = pos.getZ() - 24; z < pos.getZ() + 25; z++) {
					...
					if (!ret) {
						if (world.getBiome(pos1) instanceof IDreadlandsBiome) ret = true;
					} else {
			...
			for (int z = pos.getZ() - range; z < pos.getZ() + range + 1; z++) {
				...
				Scheduler.schedule(new ScheduledProcess(num * 2) {
					...
					public void execute() {
						...
						for (EntityLivingBase e : world.getEntitiesWithinAABB(
								EntityLivingBase.class, new AxisAlignedBB(pos1).grow(3, 128, 3),
								Predicates.and(EntitySelectors.IS_ALIVE,
